...
import com.example.koch.databinding.ActivityCreateTestBinding
            ...
            try {
                if(currentName.isEmpty() && currentNumberQues==0 && currentSub.isEmpty()){
                    ...
                    if(listQuest.size!=currentNumberQues){
                        ...
                        Log.d(ContentValues.TAG,"ссылка "+database)
                        database.child("Subject").child(currentSub).addListenerForSingleValueEvent(object :
                            ValueEventListener {
                            override fun onDataChange(snapshot: DataSnapshot) {

                                if(snapshot.child(currentName).exists()){
                                    Toast.makeText(this@CreateTest,"Тест с таким именем уже есть",
